...
$(document).ready(function() {
  ...
  function renderGraph(data, dusers, marg) {
    ...
    if(viewMode == "user") {
      ...
      createTotalBar(svg, nprojs, "projbar", marg, w - marg.left - marg.right, d3.select("#userbar-0").node().getBBox().height + (marg.bot+hscl), wscl, hscl);
  ...
  function createTotalBar(svg, dat, tclass, marg, inx, iny, wscl, hscl) {
    ...
    var totalbarrow = usrbar
      .append('g')
      .attr('transform', (d, i) => { return "translate(0,"+(usrbar.node().getBBox().height + 5)+")"; });
