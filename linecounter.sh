...
a=$(find "$BASEDIR" -not \( -name node_modules -prune -o -name lib -prune  \) -iregex '.*\.\(js\|json\|ejs\|html\|php\|sql\|css\|c\|cpp\|cs\|env\|sh\)' -exec sh -c 'cat "$1" | wc -l' - {} \;);b="$(echo "$a" | tr " " "\n")";readarray -t c <<<"$b";d=0;for i in "${c[@]}";do d=$((i+d));done;echo "$d"
...
printf "\nTotal (with package-lock and node modules and frontend libs):\n"
